using API_GruasUCAB.Core.Infrastructure.KeycloakRequestBuilder.DTO;
...
namespace API_GruasUCAB.Core.Infrastructure.KeycloakRequestBuilder
{
     public class KeycloakRequestBuilder : IKeycloakRequestBuilder
     {
          ...
          public StringContent BuildUserCreationJson()
          {
          ...
          public StringContent BuildPasswordChangeJson()
          {
               var json = JsonSerializer.Serialize(_newPassword ?? new Dictionary<string, object>());
               return new StringContent(json, Encoding.UTF8, "application/json");
          }

          public StringContent BuildPasswordChangeJsonWithCredentials()
          {
               var json = JsonSerializer.Serialize(new
               {
          ...
          public StringContent BuildAssignRoleJson(string roleId, string roleName)
          {
               var json = JsonSerializer.Serialize(new[]
               {
